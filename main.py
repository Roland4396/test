...
def main():
    ...
    hrank_trigger_round = args.hrank_trigger_round if hasattr(args, 'hrank_trigger_round') else -1 
    ...
    for round_idx in range(args.start_round, total_rounds):
        ...
        # 检查是否触发 HRank
        if round_idx == hrank_trigger_round:
            print(f"\nINFO: >>>>> Triggering HRank mask generation at the beginning of round {round_idx} <<<<<")
            
            hrank_batch_size = args.hrank_representative_batch_size if hasattr(args, 'hrank_representative_batch_size') else batch_size 
            hrank_dataset_source = val_set if val_set is not None else test_set
            if hrank_dataset_source is None: hrank_dataset_source = train_set
