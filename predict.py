    ...
    with torch.no_grad():
        for i, (inp, target) in enumerate(val_loader):
            ...
            for model_idx, model in enumerate(models):
                ...
                # --- 核心修改 ---
                # The model now returns (preds, loss_rank, loss_pruning_target).
                # For validation, we only need the predictions (preds).
                # We pass target_pruning_ratio=0.0 and other params as default
                # to ensure extra losses are not computed.
                output, _, _ = model(inp, manual_early_exit_index=exit_idxs[model_idx])
                # --- 核心修改结束 ---
                ...
                for j in range(len(output)):
                    ...
                    # Ensure top1 and top5 lists are correctly indexed
                    if j < len(top1[model_idx]):
