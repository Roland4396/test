...
def modify_args(args):
    ...
    if not hasattr(args, "save_path") or args.save_path is None:
        args.save_path = f"outputs/自学习阶段"
    # Add the number of model levels based on client split configuration
    args.num_model_levels = len(args.client_split_ratios)
    return args
...
fl_group.add_argument('-trs', '--track_running_stats', action='store_true',
                      help='trs')
# ---- Pruning and Rank Loss Settings ----
pruning_group = arg_parser.add_argument_group('pruning', 'Pruning and Rank Loss Settings')
pruning_group.add_argument('--beta', type=float, default=0.1,
                           help='Strength of the rank guidance loss')
pruning_group.add_argument('--delta', type=float, default=0.45,
                           help='Strength of the pruning target loss')
pruning_group.add_argument('--target-pruning-ratios', type=float, nargs='*', default=[0.67, 0.6, 0.45, 0.0],
                           help='List of FLOPs saving targets for each model level, from smallest to largest model')
pruning_group.add_argument('--rank-sub-batch-size', type=int, default=4,
                           help='Sub-batch size for nuclear norm calculation to manage cost')
